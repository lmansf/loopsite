/**
 * tests/accounts/dog.spec.ts — one file per account. **OWNED BY WP-C.**
 * Spec: §H.2, last entry.
 *
...
const SLUG = 'dog';
const TITLE = "the dog";
const ASK = "ask the streetlight";
const NEXT = 'lamp';
const NEXT_TITLE = "the streetlight";

test('dog is in the raw response body', async ({ request }) => {
  const html = await (await request.get('/')).text();
  expect(html).toContain(`id="section-${SLUG}"`);
  expect(html).toContain(`href="/?s=${SLUG}"`);
});

test('dog activates on its own URL and nothing else does', async ({ page }) => {
...
test('dog names what it hands on', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
...
test('dog reads from an empty key set', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
  const text = await page.locator(`#section-${SLUG} .blocks`).innerText();
  expect(text.trim().length).toBeGreaterThan(60);
});

test('dog is where the reader is, in its own night', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
...
test('dog can be left and come back to', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
