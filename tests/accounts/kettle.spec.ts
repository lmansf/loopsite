/**
 * tests/accounts/kettle.spec.ts — one file per account. **OWNED BY WP-C.**
 * Spec: §H.2, last entry.
 *
...
const SLUG = 'kettle';
const TITLE = "the kettle";
const ASK = "ask the moth";
const NEXT = 'moth';
const NEXT_TITLE = "the moth";

test('kettle is in the raw response body', async ({ request }) => {
  const html = await (await request.get('/')).text();
  expect(html).toContain(`id="section-${SLUG}"`);
  expect(html).toContain(`href="/?s=${SLUG}"`);
});

test('kettle activates on its own URL and nothing else does', async ({ page }) => {
...
test('kettle names what it hands on', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
...
test('kettle reads from an empty key set', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
  const text = await page.locator(`#section-${SLUG} .blocks`).innerText();
  expect(text.trim().length).toBeGreaterThan(60);
});

test('kettle is where the reader is, in its own night', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
...
test('kettle can be left and come back to', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
