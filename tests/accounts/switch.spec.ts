/**
 * tests/accounts/switch.spec.ts — one file per account. **OWNED BY WP-C.**
 * Spec: §H.2, last entry.
 *
...
const SLUG = 'switch';
const TITLE = "the switch";
const ASK = "ask the road";
const NEXT = 'road';
const NEXT_TITLE = "the road";

test('switch is in the raw response body', async ({ request }) => {
  const html = await (await request.get('/')).text();
  expect(html).toContain(`id="section-${SLUG}"`);
  expect(html).toContain(`href="/?s=${SLUG}"`);
});

test('switch activates on its own URL and nothing else does', async ({ page }) => {
...
test('switch names what it hands on', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
...
test('switch reads from an empty key set', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
  const text = await page.locator(`#section-${SLUG} .blocks`).innerText();
  expect(text.trim().length).toBeGreaterThan(60);
});

test('switch is where the reader is, in its own night', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
...
test('switch can be left and come back to', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
