/**
 * tests/accounts/river.spec.ts — one file per account. **OWNED BY WP-C.**
 * Spec: §H.2, last entry.
 *
...
const SLUG = 'river';
const TITLE = "the river";
const ASK = "ask the last bus";
const NEXT = 'bus';
const NEXT_TITLE = "the last bus";

test('river is in the raw response body', async ({ request }) => {
  const html = await (await request.get('/')).text();
  expect(html).toContain(`id="section-${SLUG}"`);
  expect(html).toContain(`href="/?s=${SLUG}"`);
});

test('river activates on its own URL and nothing else does', async ({ page }) => {
...
test('river names what it hands on', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
...
test('river reads from an empty key set', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
  const text = await page.locator(`#section-${SLUG} .blocks`).innerText();
  expect(text.trim().length).toBeGreaterThan(60);
});

test('river is where the reader is, in its own night', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
...
test('river can be left and come back to', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
