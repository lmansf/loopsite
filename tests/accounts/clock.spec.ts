/**
 * tests/accounts/clock.spec.ts — one file per account. **OWNED BY WP-C.**
 * Spec: §H.2, last entry.
 *
...
const ASK = "ask the window";
const NEXT = 'window';
const NEXT_TITLE = "the window";

test('clock is in the raw response body', async ({ request }) => {
  const html = await (await request.get('/')).text();
  expect(html).toContain(`id="section-${SLUG}"`);
  expect(html).toContain(`href="/?s=${SLUG}"`);
});

test('clock activates on its own URL and nothing else does', async ({ page }) => {
...
test('clock names what it hands on', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
...
test('clock reads from an empty key set', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
  const text = await page.locator(`#section-${SLUG} .blocks`).innerText();
  expect(text.trim().length).toBeGreaterThan(60);
});

test('clock is where the reader is, in its own night', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
...
test('clock can be left and come back to', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
