/**
 * tests/accounts/moth.spec.ts — one file per account. **OWNED BY WP-C.**
 * Spec: §H.2, last entry.
 *
...
const SLUG = 'moth';
const TITLE = "the moth";
const ASK = "ask the river";
const NEXT = 'river';
const NEXT_TITLE = "the river";

test('moth is in the raw response body', async ({ request }) => {
  const html = await (await request.get('/')).text();
  expect(html).toContain(`id="section-${SLUG}"`);
  expect(html).toContain(`href="/?s=${SLUG}"`);
});

test('moth activates on its own URL and nothing else does', async ({ page }) => {
...
test('moth names what it hands on', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
...
test('moth reads from an empty key set', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
  const text = await page.locator(`#section-${SLUG} .blocks`).innerText();
  expect(text.trim().length).toBeGreaterThan(60);
});

test('moth is where the reader is, in its own night', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
...
test('moth can be left and come back to', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
