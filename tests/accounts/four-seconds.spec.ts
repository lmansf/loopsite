...
const TITLE = "four seconds";
const ASK = "ask the dog again";
const NEXT = 'dog';
const NEXT_TITLE = "the dog";
...
test('four-seconds is where the reader is, in its own night', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
...
test('four-seconds can be left and come back to', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
