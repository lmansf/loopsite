/**
 * tests/accounts/road.spec.ts — one file per account. **OWNED BY WP-C.**
 * Spec: §H.2, last entry.
 *
...
const SLUG = 'road';
const TITLE = "the road";
const ASK = "ask the four seconds";
const NEXT = 'four-seconds';
const NEXT_TITLE = "four seconds";

test('road is in the raw response body', async ({ request }) => {
  const html = await (await request.get('/')).text();
  expect(html).toContain(`id="section-${SLUG}"`);
  expect(html).toContain(`href="/?s=${SLUG}"`);
});

test('road activates on its own URL and nothing else does', async ({ page }) => {
...
test('road names what it hands on', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
...
test('road reads from an empty key set', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
  const text = await page.locator(`#section-${SLUG} .blocks`).innerText();
  expect(text.trim().length).toBeGreaterThan(60);
});

test('road is where the reader is, in its own night', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
...
test('road can be left and come back to', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
