/**
 * tests/accounts/radio.spec.ts — one file per account. **OWNED BY WP-C.**
 * Spec: §H.2, last entry.
 *
...
const SLUG = 'radio';
const TITLE = "the radio";
const ASK = "ask the clock";
const NEXT = 'clock';
const NEXT_TITLE = "the clock";

test('radio is in the raw response body', async ({ request }) => {
  const html = await (await request.get('/')).text();
  expect(html).toContain(`id="section-${SLUG}"`);
  expect(html).toContain(`href="/?s=${SLUG}"`);
});

test('radio activates on its own URL and nothing else does', async ({ page }) => {
...
test('radio names what it hands on', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
...
test('radio reads from an empty key set', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
  const text = await page.locator(`#section-${SLUG} .blocks`).innerText();
  expect(text.trim().length).toBeGreaterThan(60);
});

test('radio is where the reader is, in its own night', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
...
test('radio can be left and come back to', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
