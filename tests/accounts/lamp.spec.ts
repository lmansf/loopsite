/**
 * tests/accounts/lamp.spec.ts — one file per account. **OWNED BY WP-C.**
 * Spec: §H.2, last entry.
 *
...
const SLUG = 'lamp';
const TITLE = "the streetlight";
const ASK = "ask the kettle";
const NEXT = 'kettle';
const NEXT_TITLE = "the kettle";

test('lamp is in the raw response body', async ({ request }) => {
  const html = await (await request.get('/')).text();
  expect(html).toContain(`id="section-${SLUG}"`);
  expect(html).toContain(`href="/?s=${SLUG}"`);
});

test('lamp activates on its own URL and nothing else does', async ({ page }) => {
...
test('lamp names what it hands on', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
...
test('lamp reads from an empty key set', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
  const text = await page.locator(`#section-${SLUG} .blocks`).innerText();
  expect(text.trim().length).toBeGreaterThan(60);
});

test('lamp is where the reader is, in its own night', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
...
test('lamp can be left and come back to', async ({ page }) => {
  await page.goto(`/?s=${SLUG}`);
